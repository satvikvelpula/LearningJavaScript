...
for (let i of picArray) {
  ...
  h2_title.innerHTML = i.title

  const desc = document.createElement('p')
  desc.innerHTML = i.description

  const article_div = document.createElement('article')
  article_div.id = "article_id"
  article_div.appendChild(h2_title)
  article_div.appendChild(desc)

  /* Image and Figure Caption is in Figure div */
  ...
  article_div.appendChild(figure)

  targetSection.appendChild(article_div)

  const modal = document.querySelector('dialog')
  const modal_close_button = document.querySelector('dialog span')
  modal.close()

  article_div.addEventListener('click', () => {
    const modal_img = document.querySelector('dialog img')
    modal_img.src = i.image.large
    modal_img.alt = i.title
...
// Modal should pop up with large image when get_article is clicked. (addEventListener)
